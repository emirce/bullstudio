...
BullMQ requires each queue's keys to share a cluster slot. You can achieve
...
`--base-path` is not supported with `--dev`: the Vite dev server used in
development mode serves assets directly and does not apply the rewrites.
