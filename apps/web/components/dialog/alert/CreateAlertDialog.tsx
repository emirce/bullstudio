...
const ALERT_TYPE_OPTIONS = [
  {
    value: AlertType.FailureRate,
    label: "Failure Rate",
    description: "Alert when failure percentage exceeds threshold",
  },
  {
    value: AlertType.BacklogExceeded,
    label: "Backlog Exceeded",
    description: "Alert when waiting + delayed jobs exceed threshold",
  },
  {
    value: AlertType.ProcessingTimeAvg,
    label: "Avg Processing Time",
    description: "Alert when average processing time exceeds threshold",
  },
  {
    value: AlertType.ProcessingTimeP95,
    label: "P95 Processing Time",
    description: "Alert when P95 processing time exceeds threshold",
  },
  {
    value: AlertType.ProcessingTimeP99,
    label: "P99 Processing Time",
    description: "Alert when P99 processing time exceeds threshold",
  },
  {
    value: AlertType.MissingWorkers,
    label: "Missing Workers",
    description: "Alert when no workers are processing the queue",
  },
];
...
export function CreateAlertDialog({
  ...
  const onSubmit = (data: CreateAlertFormValues) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Set up an alert to monitor queue health metrics
          </DialogDescription>
        </DialogHeader>
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                ...
                <Input {...field} placeholder="High failure rate alert" />
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                ...
                <Textarea
                  {...field}
                  placeholder="Alert triggered when..."
              ...
              render={({ field, fieldState }) => (
                <Field data-invalid={fieldState.invalid}>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Evaluate metrics over this time period (1-60 min)
                  </p>
                  {fieldState.error && (
                    <FieldError errors={[fieldState.error]} />
                  )}
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                ...
                />
                <p className="text-xs text-muted-foreground mt-1">
                  Minimum time between notifications for repeated triggers
                </p>
                {fieldState.error && <FieldError errors={[fieldState.error]} />}
              </Field>
            )}
